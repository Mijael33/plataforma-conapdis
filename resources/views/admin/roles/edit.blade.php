...
<div class="container-fluid">
    <h2 style="color: #1a3b5d; font-weight: 700;" class="mb-4">Editar Rol: {{ $rol->nombre }}</h2>

    <div class="card border-0 shadow-sm rounded-4 p-4">
        <form action="{{ route('admin.roles.update', $rol) }}" method="POST" id="formRol">
            @csrf
            @method('PUT')
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <label class="form-label fw-bold">Nombre del Rol *</label>
                    <input type="text" name="nombre" class="form-control rounded-3" value="{{ old('nombre', $rol->nombre) }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-bold">Descripción</label>
                    <input type="text" name="descripcion" class="form-control rounded-3" value="{{ old('descripcion', $rol->descripcion) }}">
                </div>
                <div class="col-md-2">
                    <div class="form-check mt-4">
                        <input type="checkbox" name="es_admin" id="es_admin" value="1" class="form-check-input" {{ $rol->es_admin ? 'checked' : '' }}>
                        <label class="form-check-label fw-bold" for="es_admin">👑 Admin Total</label>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-check mt-4">
                        <input type="checkbox" name="activo" id="activo" value="1" class="form-check-input" {{ $rol->activo ? 'checked' : '' }}>
                                ...
                                @if($areaKey === 'dashboard')
                                    ...
                                    <div class="form-check">
                                        <input type="checkbox" 
                                               ...
                                               {{ isset($permisosRol[$areaKey]) && in_array($accion, $permisosRol[$areaKey]) ? 'checked' : '' }}>
