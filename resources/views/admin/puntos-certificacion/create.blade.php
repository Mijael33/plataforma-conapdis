...
@section('titulo', 'Nuevo Punto de Certificación')

@section('contenido')
<div class="container-fluid">
    <h2 style="color: #1a3b5d; font-weight: 700;" class="mb-4">Nuevo Punto de Certificación</h2>

    <div class="card border-0 shadow-sm rounded-4 p-4">
        <form action="{{ route('admin.puntos-certificacion.store') }}" method="POST">
            @csrf
            <div class="row g-3">
                <div class="col-md-3">
                    <label class="form-label fw-bold">Estado</label>
                    <input type="text" name="estado" class="form-control rounded-3" value="{{ old('estado') }}" required placeholder="Ej: Distrito Capital">
                </div>
                <div class="col-md-3">
                    <label class="form-label fw-bold">Fecha</label>
                    <input type="date" name="fecha" class="form-control rounded-3" value="{{ old('fecha', date('Y-m-d')) }}" required>
                </div>
                <div class="col-md-2">
                    <label class="form-label fw-bold">Hora Inicio</label>
                    <input type="time" name="hora" class="form-control rounded-3" value="{{ old('hora') }}">
                </div>
                <div class="col-md-2">
                    <label class="form-label fw-bold">Hora Fin</label>
                    <input type="time" name="hora_fin" class="form-control rounded-3" value="{{ old('hora_fin') }}">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-12">
                    ...
                    <div class="d-flex flex-wrap gap-3">
                        ...
                        <div class="form-check">
                            <input type="checkbox" name="dias[]" value="{{ $dia }}" class="form-check-input" id="dia_{{ $loop->index }}" {{ in_array($dia, $diasOld) ? 'checked' : '' }}>
                ...
                <div class="col-12">
                    ...
                    <div id="ubicaciones-container">
                        @if(old('ubicaciones'))
                            @foreach(old('ubicaciones') as $ubi)
