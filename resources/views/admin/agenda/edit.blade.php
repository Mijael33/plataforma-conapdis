...
<div class="container-fluid">
    <h2 style="color: #1a3b5d; font-weight: 700;" class="mb-4">Editar Evento</h2>
        ...
        <form action="{{ route('admin.agenda.update', $agenda) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="titulo" class="form-control rounded-3" value="{{ old('titulo', $agenda->titulo) }}" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label fw-bold">Fecha *</label>
                    <input type="date" name="fecha" class="form-control rounded-3" value="{{ old('fecha', $agenda->fecha->format('Y-m-d')) }}" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label fw-bold">Hora</label>
                    <input type="text" name="hora" class="form-control rounded-3" value="{{ old('hora', $agenda->hora) }}">
                </div>
                <div class="col-12">
                    <label class="form-label fw-bold">Lugar</label>
                    <input type="text" name="lugar" class="form-control rounded-3" value="{{ old('lugar', $agenda->lugar) }}">
                </div>
                <div class="col-12">
                    <label class="form-label fw-bold">Extracto</label>
                    <textarea name="extracto" class="form-control rounded-3" rows="2">{{ old('extracto', $agenda->extracto) }}</textarea>
                </div>
                <div class="col-12">
                    <label class="form-label fw-bold">Contenido (Editor enriquecido)</label>
                    <textarea name="contenido" id="editor" class="form-control rounded-3" rows="8">{{ old('contenido', $agenda->contenido) }}</textarea>
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-bold">Imagen destacada</label>
                    @if($agenda->imagen)
                    <div class="mb-2"><img src="{{ asset('storage/' . $agenda->imagen) }}" alt="Imagen actual" style="max-width: 200px; border-radius: 8px;"></div>
                    @endif
                    <input type="file" name="imagen" class="form-control rounded-3">
                </div>
                <div class="col-12">
                    <div class="form-check">
                        <input type="checkbox" name="publicado" value="1" class="form-check-input" {{ $agenda->publicado ? 'checked' : '' }}>
                        <label class="form-check-label fw-bold">Publicado</label>
                    </div>
                </div>
                <div class="col-12">
                    <button type="submit" class="btn btn-success rounded-pill px-4">Actualizar Evento</button>
                    <a href="{{ route('admin.agenda.index') }}" class="btn btn-outline-secondary rounded-pill px-4">Cancelar</a>
                </div>
            </div>
        </form>
    </div>
